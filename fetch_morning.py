...
def main():
    ...
    # ── Step 5.5: 先算"实际新增数"（2026-09-12 fix：必须在覆盖缓存之前；base 一层 dirname = 仓库根）──
    from lib.dedup import load_url_cache
    base = os.path.dirname(os.path.abspath(__file__))
    prev_urls = load_url_cache(os.path.join(base, output["url_cache_file"]))
    new_count = len(set(all_urls) - prev_urls) if prev_urls else total_count
    ...
    if repeat_check == "no_new":
        # 2026-09-12 fix: new_count 已在上方（覆盖缓存前）算好，直接复用
